...
EXPOSE 8080
CMD streamlit run apps/streamlit/app.py --server.port=${PORT:-8080} --server.address=0.0.0.0
```
...
- 로컬에서 미리 빌드해보고 싶다면:
  ```
  docker build -f infrastructure/Dockerfile -t invest-assistant-ui .
...
```
python -c "from invest_assistant.storage.drive import DriveDB; print(DriveDB().list_tickers())"
```
