...
직전 현재 명세는 [v3.64 스냅샷](archive/investment_assistant_spec_v3.64.md)에 보존한다.
...
목표 배포 구조는 GCP Compute Engine의 Streamlit + Cloudflare DNS·Access·Tunnel이다.
상세 요구사항과 인수 기준은 [GCP·Cloudflare 운영 명세 v1.0](streamlit_gcp_cloudflare_spec.md)을 따른다.
초기 사용자는 운영자 본인 1명이며, 실제 도메인·GCP 리소스·비용은 배포 전에 확정한다.
