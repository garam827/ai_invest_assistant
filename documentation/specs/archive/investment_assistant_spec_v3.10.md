...
| v3.10 | 2026-07-21 | **크론 분(分)을 `:37` → `:57`로 재조정(사용자 직접 수정).** 겨울철(EST) 버퍼를 조금 더 확보하고 싶어 `cron: "37 21 * * 1-5"`에서 `"57 21 * * 1-5"`로 20분 늦췄다 — 한국시간 도착 시각은 06:37경 → 06:57경으로, EST 버퍼는 약 37분 → 약 57분으로 늘었다. 정각/반각을 피한다는 원칙은 그대로 유지(`:57`도 비정형 분). [기능 5] 갱신 |
