...
- **통합 일일 리포트 (`report_builder.py`, v3.0 신설)**: 그날의 11종목 판정 결과 하나로 HTML 리포트 한 장을 만든다(`build_daily_report_html`).
  ...
  - **데스크톱 여백/뉴스 카드 레이아웃 (v3.3)**: 컨테이너 `max-width`를 1000px → 1400px(`width: 94%`)로 넓혀 PC 화면에서 좌우 여백이 과도했던 문제를 줄이고, 뉴스 카드는 세로 한 줄 나열 대신 CSS 그리드(`repeat(auto-fit, minmax(320px, 1fr))`)로 배치해 넓어진 화면에서는 여러 열로, 좁은 화면(모바일)에서는 자동으로 한 열로 줄어든다.
