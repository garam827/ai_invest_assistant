...
1. **매일 장 마감 후**: GitHub Actions `collect.yml`이 스케줄(UTC 기준 일~목 22:30 = 한국시간 월~금 07:30, v3.7)로 가동.
...
- **워크플로 2개, `workflow_run`으로 연쇄**:
  - `.github/workflows/collect.yml`: `cron: "30 22 * * 0-4"`(UTC 일~목 22:30) + `workflow_dispatch`(수동 실행)로 가동. `python data_fetcher.py update` 한 줄로 `sync_universe`+전 종목 갱신+자산군 ETF 갱신을 모두 수행. **요일 마스크는 UTC 기준**이라 한국시간(UTC+9)으로 환산하면 자정을 넘어가 하루씩 밀린다는 점에 주의 — UTC 월~금(`1-5`)으로 두면 한국시간으로는 화~토 아침에 알림이 가서(월요일 아침 알림 누락 + 불필요한 토요일 알림 발생) v3.7에서 UTC 일~목(`0-4`)로 수정했다. UTC 일요일 실행은 그 시점에 아직 새 미국 거래일 데이터가 없어 지난 금요일 종가를 다시 확인하는 것과 같은데, 이게 결과적으로 "월요일 아침 캐치업" 역할을 한다.
