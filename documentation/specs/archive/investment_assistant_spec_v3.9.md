...
1. **매일 장 마감 후**: GitHub Actions `collect.yml`이 스케줄(UTC 기준 월~금 21:37 = 한국시간 화~토 06:37경, v3.9 — 아래 참고)로 가동.
...
- **워크플로 2개, `workflow_run`으로 연쇄**:
  - `.github/workflows/collect.yml`: `cron: "37 21 * * 1-5"`(UTC 월~금 21:37) + `workflow_dispatch`(수동 실행)로 가동. `python data_fetcher.py update` 한 줄로 `sync_universe`+전 종목 갱신+자산군 ETF 갱신을 모두 수행. **요일 마스크는 UTC 기준**이라 한국시간(UTC+9)으로 환산하면 자정을 넘어가 하루씩 밀려 한국시간으로는 **화~토 06:37경**에 도착한다(v3.7에서 "월요일 아침 알림이 없다"는 이유로 UTC 일~목으로 바꿔봤으나, 재검토 결과 한국시간 월요일 아침 시점엔 어차피 새 미국 거래일 데이터가 아직 없어(주말 지난 금요일 종가 그대로) 알림을 보내봤자 중복이라는 결론으로 v3.8에서 UTC 월~금으로 원복 — 토요일 알림(금요일 종가)은 그 주의 마지막 거래일 데이터를 확인하는 의미가 있어 유지).
  - **시각을 06:37 KST로 앞당김 + 정각/반각 회피 (v3.9)**: 기존 22:30 UTC(한국시간 07:30경)에서 21:37 UTC로 앞당겨 한국시간 기상 시간에 더 맞췄다. 다만 이 때문에 EST(미국 겨울시간, 11~3월) 구간에는 장마감(16:00 ET) 후 버퍼가 **약 37분**으로 줄어든다(기존 22:30 UTC는 EST 기준 1.5시간 버퍼를 일부러 확보한 설계였음) — 그 시점에 yfinance 데이터가 아직 안 올라와 있으면 그날치만 못 가져오고 다음 실행에서 자가 치유되는 정도라 감수 가능한 트레이드오프로 판단. 분(分)도 `:37`처럼 정각/반각이 아닌 값으로 의도적으로 어긋나게 잡았다 — GitHub Actions가 `:00`/`:15`/`:30`/`:45` 같은 인기 있는 시각에 스케줄 실행을 더 많이 큐잉해 지연이 잦다는 게 알려져 있기 때문(완전히 피할 수는 없고 확률만 줄이는 수준).
