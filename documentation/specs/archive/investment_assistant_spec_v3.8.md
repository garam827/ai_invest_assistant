...
| v3.8 | 2026-07-21 | **크론 요일 마스크를 v3.7 이전(UTC 월~금)으로 원복 + GitHub Actions 지연 특성 문서화.** v3.7에서 "한국시간 월요일 아침 알림이 없다"는 이유로 UTC 일~목(`0-4`)로 바꿨었는데, 재검토 결과 한국시간 월요일 아침 시점엔 미국 시장이 아직 그 주의 거래를 시작하지 않아(주말 지난 금요일 종가 그대로) 알림을 보내도 중복 정보라는 결론에 도달 — `cron: "30 22 * * 1-5"`(UTC 월~금, 한국시간 화~토 07:30)로 되돌렸다. 별개로, "왜 07:30이 아니라 08:30에 실행됐냐"는 질문에서 GitHub Actions 스케줄이 예정 시각(22:30 UTC)보다 약 61분 늦게(23:31 UTC) 실행된 실제 사례를 발견 — 크론 표현식 문제가 아니라 GitHub Actions 스케줄 자체가 "best-effort"라 사용량이 몰리는 정각/반각 근처에는 지연될 수 있다는, 공식적으로 알려진 플랫폼 특성. `collect.yml` 주석과 [기능 5]에 명문화. [기능 5] 갱신 |
...
1. **매일 장 마감 후**: GitHub Actions `collect.yml`이 스케줄(UTC 기준 월~금 22:30 = 한국시간 화~토 07:30, v3.8에서 최종 확정 — 아래 참고)로 가동.
...
- **워크플로 2개, `workflow_run`으로 연쇄**:
  - `.github/workflows/collect.yml`: `cron: "30 22 * * 1-5"`(UTC 월~금 22:30) + `workflow_dispatch`(수동 실행)로 가동. `python data_fetcher.py update` 한 줄로 `sync_universe`+전 종목 갱신+자산군 ETF 갱신을 모두 수행. **요일 마스크는 UTC 기준**이라 한국시간(UTC+9)으로 환산하면 자정을 넘어가 하루씩 밀려 한국시간으로는 **화~목 07:30**에 도착한다(v3.7에서 "월요일 아침 알림이 없다"는 이유로 UTC 일~목으로 바꿔봤으나, 재검토 결과 한국시간 월요일 아침 시점엔 어차피 새 미국 거래일 데이터가 아직 없어(주말 지난 금요일 종가 그대로) 알림을 보내봤자 중복이라는 결론으로 v3.8에서 UTC 월~금으로 원복 — 토요일 알림(금요일 종가)은 그 주의 마지막 거래일 데이터를 확인하는 의미가 있어 유지).
