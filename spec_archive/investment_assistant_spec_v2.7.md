...
6. **LLM 브리핑**: 시그널(매수/매도) 발생 종목의 최근 뉴스를 Exa API로 수집해 OpenRouter LLM(Nemotron)이 팩트 기반 요약 및 매수/HOLD/매도 추천.
