...
from invest_assistant.paths import ARTIFACT_DIR

BACKTEST_CACHE_DIR = str(ARTIFACT_DIR / "cache" / "backtest")
...
def run_backtest(tickers: dict | None = None, analysis_start: str = ANALYSIS_START) -> pd.DataFrame:
    """tickers defaults to instrument_universe.ASSET_CLASS_TICKERS. Combines fetch_backtest_history +
...
def build_full_universe(drive_db) -> dict:
    """instrument_universe.ASSET_CLASS_TICKERS (12) + every active S&P 500 ticker from Drive's
    ...
    combined = dict(instrument_universe.ASSET_CLASS_TICKERS)
