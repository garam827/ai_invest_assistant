...
DOCS_DATA_DIR = str(SITE_DIR / "data")
...
def export_signals_json(results: dict, sp500_signals: list[dict], signal_history: dict | None = None, overview: str = "") -> None:
...
def export_universe_json(drive_db: DriveDB) -> None:
    ...
    universe = drive_db.load_json(UNIVERSE_FILENAME) or {}
    ...
    payload = {
        "asset_classes": [
            ...
            for t, m in instrument_universe.ASSET_CLASS_TICKERS.items()
        ],
        "sp500": [
...
def export_reports_index(history: dict | None = None) -> None:
    ...
    (report_store.save_report writes there) -- no extra Drive round-trip needed. "_test"
    -suffixed dates are excluded, same as report_store.list_report_dates.
    ...
    reports_dir = report_store.LOCAL_REPORT_DIR
...
def export_chart_data(drive_db: DriveDB, tickers: list[str] | None = None) -> None:
    ...
    if tickers is None:
        universe = drive_db.load_json(UNIVERSE_FILENAME) or {}
        tickers = list(instrument_universe.ASSET_CLASS_TICKERS) + sorted(universe.get("active_tickers", []))
