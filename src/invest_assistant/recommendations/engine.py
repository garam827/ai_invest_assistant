"""Per-ticker mechanical recommendations, optional narrative, and signal history.

The daily publication flow lives in invest_assistant.pipelines.recommend.
News/LLM failure falls back to a rule-based explanation without changing the action.
"""
...
from invest_assistant.data import news as news_fetcher
...
def backfill_signal_history(drive_db: DriveDB) -> dict:
    ...
    dates are excluded, same as report_store.list_report_dates. Only covers the (short)
...
def get_recommendation_for_ticker(
    ...
    try:
        ...
        if news is None:
            ...
            news_query = instrument_universe.ASSET_CLASS_TICKERS.get(ticker, {}).get("news_query")
